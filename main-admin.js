...
const QR_TEMPLATE_FLORAL_PATH = "assets/qr-template-floral.png";
const QR_TEMPLATE_MINIMAL_PATH = "assets/qr-template-minimal.png";

async function createPosterPdfFromTemplate({
  templatePath,
  qrDataUrl,
  qrX = 50.8,
  qrY = 121.6,
  qrSize = 108.4,
  fileSuffix = "qr-poster"
}) {
  const { jsPDF } = window.jspdf;
  const templateDataUrl = await imageUrlToDataUrl(templatePath);
  ...
  pdf.addImage(templateDataUrl, "PNG", 0, 0, 210, 297);
  pdf.addImage(qrDataUrl, "PNG", qrX, qrY, qrSize, qrSize);

  return { pdf, fileSuffix };
}

window.downloadPrintPdfFloral = async function () {
  if (!ensureCreatedEvent()) return;

  const event = createdEventPayload;
  const qrDataUrl = await imageUrlToDataUrl(event.qrUrl);
  const { pdf, fileSuffix } = await createPosterPdfFromTemplate({
    templatePath: QR_TEMPLATE_FLORAL_PATH,
    qrDataUrl,
    fileSuffix: "qr-poster-floral"
  });

  pdf.save(`${safeFileName(event.title)}-${fileSuffix}.pdf`);
};

window.downloadPrintPdfMinimal = async function () {
  if (!ensureCreatedEvent()) return;

  const event = createdEventPayload;
  const qrDataUrl = await imageUrlToDataUrl(event.qrUrl);
  const { pdf, fileSuffix } = await createPosterPdfFromTemplate({
    templatePath: QR_TEMPLATE_MINIMAL_PATH,
    qrDataUrl,
    fileSuffix: "qr-poster-minimal"
  });

  pdf.save(`${safeFileName(event.title)}-${fileSuffix}.pdf`);
};

// zadrzano radi kompatibilnosti sa starim pozivima
window.downloadPrintPdf = window.downloadPrintPdfFloral;
